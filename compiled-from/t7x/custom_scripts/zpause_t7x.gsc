/*
======================================================================
    ZPAUSE T7 (T7x) v1.5  --  Synced co-op pause for Black Ops III Zombies

    by Xep

======================================================================

    A port of ZPause (Plutonium T6) to Black Ops III.

    THIS IS THE T7x PACKAGING. The same script as the Workshop build, in
    the same official dialect, standing in for nothing.

        Buttons  hold crouch + melee

        Install  drop in as

                 <bo3>\t7x\custom_scripts\zpause.gsc

    T7x loads a script out of that folder by name, so nothing has to be
    overridden to reach it and no mod slot is taken -- which is why this
    build exists alongside the Workshop one. It keeps its own namespace and
    registers itself, and that is the whole difference between the two.
...
{
    /*
        Before the callbacks, not only in init(): both copies register
        their own, under different system names, and the losing copy's
        on_spawned would otherwise still fire and could win the
        self.zp_thinking race.
    */
...
*/
function set_dvar_if_unset( dvar, def )
{
...
function zp_is_unpause_word( msg )
{
...
function zp_combo_label( combo, binds )
{
    if ( zp_true( binds ) )
...
*/
function zp_player_input_limited( player )
{
    if ( !isdefined( player ) )
        return 0;
...
function zp_active_combo( up_combo, dead_combo )
{
...
*/
function zp_input_debug_watcher()
{
    self endon( "disconnect" );
    level endon( "end_game" );
...
function zp_do_unpause( player, label )
{
    level endon( "end_game" );
...
*/
function zp_engine_zombies( benable )
{
    if ( !benable )
    {
...
*/
function zp_anim_freeze_pass()
{
}

function zp_anim_thaw_pass()
{
}
...
function zp_hud_style( elem, alpha )
{
    if ( !isdefined( elem ) )
        return;
...
*/
function zp_panel_place( elem, position, pad )
{
    if ( position == "top" )
    {
...
function zp_panel_show( position, height )
{
...
function zp_hud_place( elem, position, yoff )
{
    if ( !isdefined( elem ) )
        return;
...
*/
function zp_clock_text_updater()
{
...
/*
    ...
    differently.
*/
function zp_subline_create( position, yoff, scale, alpha )
{
...
*/
function zp_pause_hint_show( elem, player )
{
...
function zp_hud_sub_show( player )
{
    if ( !isdefined( player ) )
        return;
...
function zp_down_hint_show( elem, kind )
{
...
function zp_down_line_show( position, yoff, kind )
{
...
function zp_show( elem, key, a, b )
{
    zp_hud_text( elem, zp_text( key, a, b ) );
}

function zp_text_player( player )
{
    if ( isdefined( player ) && isdefined( player.name ) )
        return player.name;
...
function zp_vote_stop( keep_title )
{
    level.zp_vote_approval = 0;
    level.zp_vote_serial = level.zp_vote_serial + 1;
    level.zp_vote_active = 0;
...
*/
function zp_vote_outcome( key, a, b )
{
...
function zp_menu_text( font, scale, alignx, x, y )
{
...
function zp_menu_section_show( elem, section )
{
    if ( section == "debug" )
...
*/
function zp_vote_hint_key( player )
{
...
function zp_vote_sub_show( player )
{
    if ( !isdefined( player ) )
        return;
...
*/
function zp_cfg_echo( dvar, value, def )
{
